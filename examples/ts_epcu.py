...
def rule_1_out(input):
    return 0.6 + (
        ...
        + 0.5 * short_term_f_set.mu(input["duration"])
        + 0.9 * high_t_experience_f_set.mu(input["tool_exp"])
        ...
        + 0.6 * low_size_f_set.mu(input["size"])
    )

def rule_2_out(input):
    return 0.8 + (
        0.7 * moderate_complexity_f_set.mu(input["complexity"])
        + 0.6 * high_t_experience_f_set.mu(input["tool_exp"])
        + 0.7 * low_risk_f_set.mu(input["risk"])
...
def rule_7_out(input):
    return 1.3 + (
        ...
        0.7 * low_size_f_set.mu(input["size"])
    )

def rule_8_out(input):
    return 1.0 + (
        0.85 * moderate_size_f_set.mu(input["size"])
...
ts_epcu = TSControl([
    ...
    TSRule([
        ...
        ( "motivation", moderate_motivation_f_set )
    ], rule_7_out),
    TSRule([
